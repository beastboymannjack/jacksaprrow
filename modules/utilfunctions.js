...
function truncateText(text, maxLength = 4096, suffix = '...') {
    if (!text) return '';
...
function truncateForEmbed(text) {
    return truncateText(text, 4096, '\n\n*[Response truncated due to length]*');
}

function truncateForField(text, maxLength = 1024) {
    return truncateText(text, maxLength, '...');
}

function isValidTicket(channel) {
    let validCategories = [
        ...(Array.isArray(mainconfig.VaildCats) ? mainconfig.VaildCats : [mainconfig.VaildCats]),
...
function theDB(client, guild) {
    ...
    const ensureTicketConfig = (path, defaultConfig) => {
        try {
            if (client.setups.has(guild.id) && !client.setups.has(guild.id, path)) {
                client.setups.set(guild.id, defaultConfig, path);
            }
        } catch (e) {
            // Silently ignore database config errors for missing guilds
        }
    };
    
    //SYSTEM BOT
    ensureTicketConfig("ticketsystem3", {
        enabled: true,
        guildid: guild.id,
    ...
    **Please tell us all of those Informations!** | Otherwise, we can't create your Bot quickly!
    ...
    > *(e.g: \`!help | discord.gg/deadloom <3\`)*
    > 
...
//REPLACE_MARKER ticketsystem2");
    //RYTHM CLONE
    ensureTicketConfig("ticketsystem6", {
        enabled: true,
        guildid: guild.id,
...
//REPLACE_MARKER ticketsystem6");
    //SOURCE CODE
    ensureTicketConfig("ticketsystem7", {
        enabled: true,
        guildid: guild.id,
...
//REPLACE_MARKER ticketsystem7");
    //Custom Bot
    ensureTicketConfig("ticketsystem8", {
        enabled: true,
        guildid: guild.id,
        parentid: `${mainconfig.TicketCategorys.ModMailBotTicketsCategory}`,
...
//REPLACE_MARKER ticketsystem8");
    //ADMINISTRATION BOT
    ensureTicketConfig("ticketsystem5", {
        enabled: true,
        guildid: guild.id,
        parentid: `${mainconfig.TicketCategorys.ModMailBotTicketsCategory}`,
...
//REPLACE_MARKER ticketsystem5");
    //Mod Mail BOT
    ensureTicketConfig("ticketsystem9", {
        enabled: true,
        guildid: guild.id,
        parentid: `${mainconfig.TicketCategorys.ModMailBotTicketsCategory}`,
    ...
    **Please tell us all of those Informations!** | Otherwise, we can't create your Bot quickly!
    ...
    > 
    > <:arrow:964989830272532501> **\` 5. \` What should be the STATUS?**
    > *(e.g: \`!help | https://discord.gg/deadloom\`)*
    > 
    > <:arrow:964989830272532501> **\` 6. \` What should be the STATUS TYPE?** 
    > *(\`PLAYING\` / \`WATCHING\` / \`LISTENING\` / \`STREAMING\` / \`COMPETING\`)*
    > 
...
//REPLACE_MARKER ticketsystem9");
    //Waitingroom BOT
    ensureTicketConfig("ticketsystem", {
        enabled: true,
        guildid: guild.id,
        parentid: `${mainconfig.ApplyTickets.PartnerApply}`,
...
//REPLACE_MARKER ticketsystem");
    //NSFW & FUN BOT
    ensureTicketConfig("ticketsystem10", {
        enabled: true,
        guildid: guild.id,
...
//REPLACE_MARKER ticketsystem10");
    //SECURITY BOT
    ensureTicketConfig("ticketsystem11", {
        enabled: true,
        guildid: guild.id,
        parentid: `${mainconfig.TicketCategorys.AllBotTicketsCategory}`,
...
//REPLACE_MARKER ticketsystem11");
    //LAVA MUSIC BOT
    ensureTicketConfig("ticketsystem12", {
        enabled: true,
        guildid: guild.id,
        parentid: `${mainconfig.TicketCategorys.AllBotTicketsCategory}`,
...
//REPLACE_MARKER ticketsystem12");
    //24/7 Music Bot
    ensureTicketConfig("ticketsystem13", {
        enabled: true,
        guildid: guild.id,
        parentid: `${mainconfig.TicketCategorys.AllBotTicketsCategory}`,
...
//REPLACE_MARKER ticketsystem13");
    //ticket counter db
    client.setups.ensure("COUNTER", {
        number: 421
    })
    //PARTNERAPPLY
    ensureTicketConfig("ticketsystem_partnerapply", {
        enabled: true,
        guildid: guild.id,
...
//REPLACE_MARKER ticketsystem_partnerapply");
    //TEAMAPPLY
    ensureTicketConfig("ticketsystem_teamapply", {
        enabled: true,
        guildid: guild.id,
...
//REPLACE_MARKER ticketsystem_teamapply");

    //GENERAL & HELP TICKETS
    ensureTicketConfig("ticketsystem4", {
        enabled: true,
        guildid: guild.id,
        parentid: `${mainconfig.TicketCategorys.AllBotTicketsCategory}`,
        message: `Hey {user}! Thanks for opening a TICKET

    > **Please tell us with what you need help with?**
    > A staff member will try to help you as soon as possible!`,
        adminroles: [Roles.SupporterRoleId, Roles.NewSupporterRoleId],
    });
//REPLACE_MARKER ticketsystem4");
}
...
async function swap_pages2(client, message, embeds) {
    ...
    let button_back = new Discord.ButtonBuilder().setStyle(Discord.ButtonStyle.Primary).setCustomId('1').setEmoji("833802907509719130").setLabel("Back")
    let button_home = new Discord.ButtonBuilder().setStyle(Discord.ButtonStyle.Danger).setCustomId('2').setEmoji("🏠").setLabel("Home")
    let button_forward = new Discord.ButtonBuilder().setStyle(Discord.ButtonStyle.Primary).setCustomId('3').setEmoji('832598861813776394').setLabel("Forward")
    const allbuttons = [new Discord.ActionRowBuilder().addComponents([button_back, button_home, button_forward])]
...
async function logAction(client, type, user, color, thumbnail, message) {
    ...
    channel.send({
        embeds: [
            new Discord.EmbedBuilder().setColor(color).setThumbnail(thumbnail ? thumbnail : null).setDescription(message.substr(0, 2000)).setFooter({ text: "ID: " + user.id, iconURL: user.displayAvatarURL({ dynamic: true }) }).setAuthor({ name: user.tag, iconURL: user.displayAvatarURL({ dynamic: true }) })
        ]
    }).catch(() => { })
}