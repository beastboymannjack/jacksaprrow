...
module.exports = client => {
    ...
    const nodestatChannelID = process.env.NODESTATS_CHANNEL_ID || mainconfig.LoggingChannelID?.BotManagementChannelID || null;
    const nodestatMessageID = process.env.NODESTATS_MESSAGE_ID || null;
    ...
    async function getLeast() {
        ...
        if (!nodestatChannelID || !nodestatMessageID) return;
        client.channels.fetch(nodestatChannelID).catch(() => null).then(channel => {
            if (!channel) return;
            channel.messages.fetch(nodestatMessageID).catch(() => null).then(msg => {
                if (!msg) return;
                ...
                const embed = new Discord.EmbedBuilder()
                    ...
                    .setAuthor({ name: "Our Nodestats", iconURL: msg.guild.iconURL({ dynamic: true }), url: "https://discord.gg/notsaksh" })
                    .setFooter({ text: "deadloom <3.dev | Order Bots NOW\nThis Message get's updated every 15 Minutes", iconURL: "https://imgur.com/jPItIw0.gif" })
                ...
                data.forEach(stat => {
                    ...
                    embed.addFields({ name: `${stat.ram != 1 ? online : offline} Server **\`${stat.key}\`**`, value: `> Ram: \`${(stat.ram * 100).toFixed(2)}% of ${stat.totalram}\`\n> Hosting Bots: \`${stat.bots}\`\n> Cores: \`${stat.cores}\`\n> Storage: \`${Math.floor(stat.storage / stat.totalstorage * 100)}% of ${formatBytes(stat.totalstorage)}\`` })
                })
