...
module.exports = client => {
        ...
        if(toDeleteChannels && toDeleteChannels.length > 0) {
            for(const deleteChannel of toDeleteChannels.map(d => d.channel)) {
                ...
                if(channel) {
                    ...
                    let parent1 = Guild.channels.cache.get(`${mainconfig.TicketCategorys.ClosedTicketOneCategory}`);
                    let parent2 = Guild.channels.cache.get(`${mainconfig.TicketCategorys.ClosedTicketTwoCategory}`)
                    let parent3 = Guild.channels.cache.get(`${mainconfig.TicketCategorys.ClosedTicketThreeCategory}`)
                    let parent4 = Guild.channels.cache.get(`${mainconfig.TicketCategorys.ClosedTicketFourCategory}`)
                    let parent5 = Guild.channels.cache.get(`${mainconfig.TicketCategorys.ClosedTicketFiveCategory}`)
                    if( (parent1 && parent1.type === ChannelType.GuildCategory && parent1.children.size > 50) && 
                        (parent2 && parent2.type === ChannelType.GuildCategory && parent2.children.size > 50) && 
                        (parent3 && parent3.type === ChannelType.GuildCategory && parent3.children.size > 50) && 
                        (parent4 && parent4.type === ChannelType.GuildCategory && parent4.children.size > 50) && 
                        (parent5 && parent5.type === ChannelType.GuildCategory && parent5.children.size > 50)
                        ...
                        create_transcript_buffer([...messageCollection.values()], channel, channel.guild).then(async path => {
                            try { // try to send the file
                                ...
                                await client.channels.fetch(`${mainconfig.LoggingChannelID.TicketLogChannelID}`).then(async ch => {
                                    try {
                                        if (!userid || userid.length < 5) throw new Error('Invalid userid');
                                        client.users.fetch(userid).then(async user => {
                                            await ch.send({
                                                embeds: [new Discord.EmbedBuilder()
                                                    .addFields({name: "Supporters:", value: `> ${ticketdata.join("\n")}`.substring(0, 1024)})
                                                    .setColor(0x5865F2).setFooter({ text: client.user.tag + " | ID: " + client.user.id+"\nTicketLog is attached to the Message!", iconURL: client.user.displayAvatarURL({ dynamic: true }) }).setDescription(`> 🔒 <@${client.user.id}> Executed: \`close\`\n> **For: ${user} \`${user.tag}\` (${userid})**\n> **Channel: \`${channel.name}\` (\`${channel.id}\`)**\n> **Category: \`${channel.parent?.name}\` (\`${channel.parentId}\`)**`)
                                                ], files: [attachment]
                                            })
                                        }).catch(async e => {
                                            console.log(e.stack ? String(e.stack).grey : String(e).grey)
                                        ...
                                        await client.users.fetch(userid).then(async user => {
                                            ...
                                            await user.send({
                                                embeds: [new Discord.EmbedBuilder()
                                                    ...
                                                    .addFields({name: `🔒 CLOSED BY:`, value: `${client.user.tag} | <@${client.user.id}>`})
                                                    .setFooter({ text: client.user.tag + " | ID: " + client.user.id+"\nTicketLog is attached to the Message!", iconURL: client.user.displayAvatarURL({ dynamic: true }) })
                                                    .addFields({name: `♨️ TYPE:`, value: `${channel.parent ? channel.parent.name : "UNKOWN"}`})
                                                ], files: [attachment]
                                            }).catch(console.log)
                                        })
                    ...
                    } else if(parent2 && parent2.type === ChannelType.GuildCategory && parent2.children.size < 50) {
                        await channel.setParent(parent2.id, {lockPermissions:false}).catch(()=>{});
                    } else if(parent3 && parent3.type === ChannelType.GuildCategory && parent3.children.size < 50) {
                        await channel.setParent(parent3.id, {lockPermissions:false}).catch(()=>{});
                    } else if(parent4 && parent4.type === ChannelType.GuildCategory && parent4.children.size < 50) {
                        await channel.setParent(parent4.id, {lockPermissions:false}).catch(()=>{});
                    } else if(parent5 && parent5.type === ChannelType.GuildCategory && parent5.children.size < 50) {
                        await channel.setParent(parent5.id, {lockPermissions:false}).catch(()=>{});
                    }
                    await channel.permissionOverwrites.set([
                        {id: channel.guild.id, deny: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages]}
                    ]).catch(()=>{});
    ...
    client.on('interactionCreate', async interaction => {
        ...
        if(interaction.customId == "closeticket"){
            interaction.reply({content: `$ **Now Closing the Ticket!**\n> Don't forget to leave <#${mainconfig.FeedBackChannelID.toString()}> and a review on our <https://de.trustpilot.com/review/deadloom>`, ephemeral: true}).catch(e => {console.warn(e.stack ? String(e.stack).grey : String(e).grey)});
            setTimeout(async() => {
                if(channel) {
                    let userid = false;
                    try {
    ...
    client.on('interactionCreate', async interaction => {
        ...
        if(interaction.customId == "TicketClaim") {
            ...
            // Helper function to safely get role position
            const getRolePosition = (roleId) => {
                if (!roleId || roleId === "") return -1;
                const role = guild.roles.cache.get(roleId);
                return role ? role.rawPosition : -1;
            };
            
            // Check if member has supporter role or higher
            const newSupporterPos = getRolePosition(Roles.NewSupporterRoleId);
            const memberPos = member.roles.highest.rawPosition;
            const isStaff = newSupporterPos === -1 ? 
                member.permissions.has(PermissionFlagsBits.ManageChannels) : 
                memberPos >= newSupporterPos;
            
            if (isStaff) {
                const founderPos = getRolePosition(Roles.FounderId);
                const coOwnerPos = getRolePosition(Roles.CoOwnerRoleId);
                const modPos = getRolePosition(Roles.ModRoleId);
                
                if(channel.name.slice(0, 5).includes("💎") && founderPos !== -1 && memberPos <= founderPos - 1) {
                    return interaction.reply({ephemeral: true, content: ":x: **You are not allowed to claim, due to the fact its a deadloom TICKET**"});
                }
                if(channel.name.slice(0, 5).includes("👑") && coOwnerPos !== -1 && memberPos <= coOwnerPos - 1) {
                    return interaction.reply({ephemeral: true, content: ":x: **You are not allowed to claim, due to the fact its a Co-Owner+ Ticket**"});
                }
                if(channel.name.slice(0, 5).includes("💠") && modPos !== -1 && memberPos <= modPos - 1) {
    ...
    if (!interaction.isStringSelectMenu()) return;
        if (interaction.message.channel.id == TicketChannelID || interaction.message.channel.id == RecoverChannelId || interaction.message.channel.id == OrderChannelID) {
            ...
            if(menuIndex.type){
                ...
                switch(menuIndex.type){
                    // DEPRECATED: Old bot type removed - using servicebots only
                    case "AIO_BOTS_DEPRECATED" : {
                        ...
                        guild.channels.create({
                            ...
                            const pricingInfo = mainconfig.PricingInfo || {};
                            ...
                            embeds: [
                                new EmbedBuilder().setColor("#5865F2")
                                    .setTitle(`Thanks for Ordering a ${menuIndex.value}! Send the Parameters!`)
                                    .setDescription(`Hey 👋 ${user}! Thanks for ordering a **${menuIndex.value}**\n\n**Please tell us all of those Informations!** | Otherwise, we can't create your Bot quickly!\n\n` + ticket.message.replace("{user}", `${user}`))
                                    .addFields({name: "**Price:**", value: `> ${pricingInfo.defaultPrice || "Check our pricing channel"}`})
                                    .addFields({name: "**Payment Methods:**", value: `> ${pricingInfo.paymentMethods || "Various payment methods available"}`})
                                    .setFooter({text: "Someone will help you soon", iconURL: "https://cdn.discordapp.com/attachments/936372059305566219/964968487250436206/unknown.png"}),
                                new EmbedBuilder().setColor(0xFFFF00)
                                    .setTitle(`${emoji.loading} A Staff Member will claim the Ticket soon!`)
                                    .setDescription(`**Dear 👋 ${user}!**\n> This Ticket will be claimed by a Staff Member as soon as possible!\n\n> *He/She/They will help you then!*\n\n> **Meanwhile, make sure to list us all Information needed!**\n\n💬 Thanks for choosing deadloom <3 ✌️`)
                                    .setFooter({text: "Thanks for choosing deadloom <3 ✌️", iconURL: "https://cdn.discordapp.com/attachments/936372059305566219/964968487250436206/unknown.png"})],
                            components: [
                            ...
                            interaction.editReply({content: `<#${ch.id}>`, embeds: [new EmbedBuilder()
                                .setColor("#5865F2")
                                .setTitle(`Your ${menuIndex.value} Order \`${ch.name}\` has been created!`)
                    ...
                    case "CUSTOMBOT" : {
                        ...
                        guild.channels.create({
                            ...
                            ch.send({content: `<@${user.id}> <:arrow:964989830272532501>  ${ticket.adminroles.map(r => `<@&${r}>`).join(" | ")} ${emoji.arrow_left} `, 
                            embeds: [
                                new EmbedBuilder().setColor("#6861fe").setTitle("📂 Thanks for ordering a Custom Bot!").setDescription(ticket.message.replace("{user}", `${user}`)),
                                new EmbedBuilder().setColor(0xFFFF00).setTitle(`${emoji.loading} A Staff Member will claim the Ticket soon!`).setDescription(`**Dear ${user}!**\n> This Ticket will be claimed by a Staff Member as soon as possible!\n\n> *He/She/They will help you then!*\n\n> **Meanwhile, make sure to list us all Information needed!**`).setFooter({text: "Thanks for choosing deadloom <3 ✌️", iconURL: "https://cdn.discordapp.com/attachments/936372059305566219/964968487250436206/unknown.png"})],
                            components: [
                                new ActionRowBuilder().addComponents([
                                    new ButtonBuilder().setStyle(Discord.ButtonStyle.Primary).setLabel("Claim").setEmoji("📝").setCustomId("TicketClaim")
                                ])
                    ...
                    case "SOURCECODE" : {
                        ...
                        guild.channels.create({
                            ...
                            ch.send({content: `<@${user.id}> <:arrow:964989830272532501>  ${ticket.adminroles.map(r => `<@&${r}>`).join(" | ")} ${emoji.arrow_left} `, embeds: [
                                new EmbedBuilder().setColor("#6861fe").setTitle("📂 Thanks for ordering a Source Code!").setDescription(ticket.message.replace("{user}", `${user}`)),                                new EmbedBuilder().setColor(0xFFFF00).setTitle(`${emoji.loading} A Staff Member will claim the Ticket soon!`).setDescription(`**Dear ${user}!**\n> This Ticket will be claimed by a Staff Member as soon as possible!\n\n> *He/She/They will help you then!*\n\n> **Meanwhile, make sure to list us all Information needed!**`).setFooter({text: "Thanks for choosing deadloom <3 ✌️", iconURL: "https://cdn.discordapp.com/attachments/936372059305566219/964968487250436206/unknown.png"})],
                                components: [
                                    new ActionRowBuilder().addComponents([
                                        new ButtonBuilder().setStyle(Discord.ButtonStyle.Primary).setLabel("Claim").setEmoji("📝").setCustomId("TicketClaim")
                                    ])
        ...
        try{
            ...
            await msgs.forEach(async msg => {
                ...
                if (msg.reactions && msg.reactions.cache.size > 0){
                    ...
                    for(const reaction of msg.reactions.cache.map(this_Code_is_by_deadloom_6969 => this_Code_is_by_deadloom_6969)){                      
