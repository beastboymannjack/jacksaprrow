...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!subCommand || !['add', 'list', 'remove'].includes(subCommand)) {
            const embed = new EmbedBuilder()
                .setColor(0xff6b6b)
                .setDescription(`Invalid usage!\n\n**Available commands:**\n\`noprefix add @user\` - Grant no-prefix access\n\`noprefix list\` - List all no-prefix users\n\`noprefix remove @user\` - Revoke no-prefix access`);
            return message.reply({
                embeds: [embed]
            });
        }
    ...
    const createListPage = async (page = 1) => {
        ...
        for (let i = 0; i < currentPageUsers.length; i++) {
            ...
            try {
                ...
                userList += contentText + '\n\n';
            } catch (error) {
                userList += `**${start + i + 1}.** [${user.username}](https://discord.com/users/${user.userId})\n\n`;
            }
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(userList.trim() || 'No users found')
