...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Show the history of recently played tracks'),
    ...
    async execute(interaction) {
        ...
        const createHistoryPage = async (page = 1) => {
            ...
            if (currentHistory.length === 0) {
                ...
                currentHistory.forEach((track, index) => {
                    if (!track) {
                        historyList += `**${start + index + 1}.** Unknown Track\n`;
                    } else {
                        ...
                        historyList += `**${start + index + 1}.** [${title.length > 45 ? title.slice(0, 42) + '…' : title}](${uri})\n${emojis.duration} \`${formatDuration(length)}\` • ${emojis.back} \`${timeAgo}\`\n\n`;
                    }
                });
            }
        ...
        const response = await interaction.reply(await createHistoryPage(1));
