...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription('Select a filter to apply to the music:')
            .setFooter({ text: 'Powered by deadloom Development' });
        ...
        collector.on('collect', async (btnInt) => {
            if (btnInt.user.id !== interaction.user.id) {
                const errorEmbed = new EmbedBuilder().setColor(0xff6b6b).setDescription(`${emojis.error} Only the command user can use these buttons!`);
                return btnInt.reply({ 
                    embeds: [errorEmbed], 
                    ephemeral: true 
                });
            ...
            if (applied) {
                ...
                const embed = new EmbedBuilder()
                .setColor(0x00d4aa)
                .setDescription(`${emojis.success} Applied **${filterId}** filter!`);
                await btnInt.reply({
                    embeds: [embed],
                    ephemeral: true
                });
