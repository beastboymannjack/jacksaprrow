    ...
    async execute(interaction) {
        ...
        collector.on('collect', async (buttonInteraction) => {
            ...
            if (newVoteCheck.passed) {
                ...
                clearVote(guild.id, 'skip');
                player.skip();
                const successEmbed = new EmbedBuilder().setColor(0x00d4aa).setDescription(`${emojis.skip} Vote passed! Skipped the current track. (${newVoteCheck.currentVotes}/${newVoteCheck.requiredVotes} votes)`);
        ...
        collector.on('end', (collected, reason) => {
            if (reason !== 'passed') {
                clearVote(guild.id, 'skip');
                const expiredEmbed = new EmbedBuilder().setColor(0xff6b6b).setDescription(`${emojis.error} Vote skip expired. Not enough votes received.`);
