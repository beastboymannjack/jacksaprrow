...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!settings.djModeEnabled || hasControlPermission(interaction, player)) {
        ...
        if (existingVote) {
            ...
            if (voteCheck.passed) {
                ...
                const embed = new EmbedBuilder()
                .setColor(0x00d4aa)
                .setDescription(`${emojis.stop} Vote passed! Stopped music and cleared the queue. (${voteCheck.currentVotes}/${voteCheck.requiredVotes} votes)`);
                return interaction.reply({ embeds: [embed] });
            }
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`${member.displayName} wants to stop the music.\nVotes needed: ${voteCheck.requiredVotes} (${settings.skipVoteThreshold}% of listeners)`);
        ...
        collector.on('collect', async (buttonInteraction) => {
            if (!buttonInteraction.member.voice.channelId || buttonInteraction.member.voice.channelId !== player.voiceChannel) {
                const errorEmbed = new EmbedBuilder().setColor(0xff6b6b).setDescription(`${emojis.error} You must be in the voice channel to vote!`);
            ...
            if (result.alreadyVoted) {
                const errorEmbed = new EmbedBuilder().setColor(0xff6b6b).setDescription(`${emojis.error} You have already voted!`);
                return buttonInteraction.reply({ embeds: [errorEmbed], ephemeral: true });
            }
            ...
            if (newVoteCheck.passed) {
                ...
                player.queue.clear();
                player.destroy();
                const successEmbed = new EmbedBuilder().setColor(0x00d4aa).setDescription(`${emojis.stop} Vote passed! Stopped music and cleared the queue. (${newVoteCheck.currentVotes}/${newVoteCheck.requiredVotes} votes)`);
            ...
            const updatedEmbed = new EmbedBuilder()
                ...
                .setDescription(`Votes: ${newVoteCheck.currentVotes}/${newVoteCheck.requiredVotes} needed`);
        ...
        collector.on('end', (collected, reason) => {
            if (reason !== 'passed') {
                clearVote(guild.id, 'stop');
                const expiredEmbed = new EmbedBuilder().setColor(0xff6b6b).setDescription(`${emojis.error} Vote stop expired. Not enough votes received.`);
