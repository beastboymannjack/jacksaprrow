...
async function main() {
    ...
    const botToken = process.argv[3] || config.BOT_TOKEN;
    
    if (!guildId) {
        console.error('❌ Guild ID is required: node emojiUploader.js <guildId> [botToken]');
        process.exit(1);
    }
    
    if (!botToken) {
        console.error('❌ Bot token is required (pass as argument or set BOT_TOKEN in .env)');
        process.exit(1);
    }
    
    try {
        const newEmojiIds = await uploadEmojis(guildId, botToken);
        const updated = await updateEmojiJson(newEmojiIds);
        
        console.log('📊 New Emoji IDs:');
